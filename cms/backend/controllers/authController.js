...
const { getGoogleOAuthTokens } = require("../utils/service");
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  } else if (req.cookies.j_cianciulli_access) {
    token = req.cookies.j_cianciulli_access;
  }
  if (!token) {
    return next(
...
exports.googleOAuthHandler = catchAsync(async (req, res) => {
  ...
  const { id_token, access_token } = getGoogleOAuthTokens(code);
...
//   if (req.cookies.j_cianciulli_access && req.cookies.j_cianciulli_rfsh) {
//     try {
//       // 1) verify token
//       const decodedAccessToken = await promisify(jwt.verify)(
//         req.cookies.j_cianciulli_access,
//         process.env.JWT_SECRET
//       );
//       console.log(decodedAccessToken);

//       const decodedRefreshToken = await promisify(jwt.verify)(
//         req.cookies.j_cianciulli_rfsh,
//         process.env.JWT_SECRET
//       );
