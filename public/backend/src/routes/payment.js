...
router.post("/create-checkout-session", async (req, res) => {
  try {
    ...
    if (
      ...
      // Still required (in cents)
      console.error(
        "Validation Failed: Missing required fields in formData.",
        formData
      );
      return res.status(400).json({
        error:
          "Missing required fields for payment session. Check schedule_selection (JSON string), package, student_name, email, totalAmount.",
      });
    }
    ...
    try {
      ...
      console.error(
        "Error parsing or mapping schedule_selection:",
        parseError,
    ...
    try {
      newBooking = new Booking({
        ...
        location: {
          // Assuming frontend sends structure matching the log
        ...
        package: formData.package, // Use renamed field
      ...
      if (dbError.name === "ValidationError") {
        ...
        return res
          .status(400)
          .json({ error: `Database validation failed: ${errors.join(", ")}` });
      }
    ...
    try {
      const line_items = [
        {
          price_data: {
            ...
            product_data: {
              name: `Booking - ${newBooking.package || "Service"}`, // Use package
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        }/payment/complete?session_id={CHECKOUT_SESSION_ID}&booking_id=${newBooking._id.toString()}`, // <-- ADD THIS
