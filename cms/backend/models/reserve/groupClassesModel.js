...
const GroupClass = require("./groupModel");

const groupClassesSchema = new mongoose.Schema(
  {
    group_class: {
...
groupClassesSchema.pre(/^find/, function (next) {
  this.populate({
    path: "instructor",
    select: "name",
  });
...
groupClassesSchema.pre("save", function (next) {
  this.courses_slug = slugify(`session-${this._id}`, { lower: true });
  next();
});
const GroupClasses = mongoose.model("GroupClasses", groupClassesSchema);
